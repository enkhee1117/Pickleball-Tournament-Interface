-- 0049 — Draw completeness: no silently unseated players.
--
-- Review finding: in 'same' mode with an odd gender cohort (and, in edge
-- cases, any mode where a player's candidate set empties), the draw's
-- continue/exit paths could leave an active player with no pairing AND no
-- sit-out row — invisible to the organizer, and skipped by the sit-out
-- fairness rotation next round. The draw now sweeps all active unseated
-- players into mixer_sit_outs after the pairing loop, so sit_out_count /
-- sat_last_round accounting (already keyed off that table below) covers
-- them, and boards render them as sitting out instead of vanishing.
--
-- Function body otherwise identical to 0047.
