-- 0052 — Admin ballot simulation: let an organizer cast ballots for players.
--
-- Testing the whole mixer flow (vote → draw → score → standings) needs many
-- players voting, but an organizer can't log in as 16 people. Every vote RPC
-- gates on app_player_belongs_to_user(voter) = (tournament_players.profile_id =
-- auth.uid()), so an organizer literally cannot vote for anyone else and no
-- simulate path exists.
--
-- Fix: factor the ballot write into an internal, auth-free core
-- (app_mixer_write_ballot) and give it two thin wrappers that share identical
-- token accounting:
--   * app_mixer_set_ballot        — the player path; still checks the caller
--                                   owns the voter roster entry (unchanged).
--   * app_mixer_admin_set_ballot  — the organizer path; checks the caller is a
--                                   tournament manager, then writes a ballot for
--                                   ANY roster entry (claimed or placeholder).
-- The internal core keeps every other guard (round open + not past lock, voter
-- in the tournament, budget, per-target cap, self/both-direction rules), so a
-- simulated ballot obeys exactly the same rules a real one does — including the
-- lock, so votes still can't be stuffed after the draw arms.
--
-- app_mixer_write_ballot is revoked from all and never granted to authenticated:
-- only the two SECURITY DEFINER wrappers (owned by postgres) call it.

set search_path = public;

create or replace function public.app_mixer_write_ballot(
  p_round_id uuid,
...
revoke all on function public.app_mixer_write_ballot(uuid, uuid, jsonb, boolean) from public;

-- Player path: unchanged behaviour — you may only write your own ballot.
...
begin
  ...
  perform public.app_mixer_write_ballot(p_round_id, p_voter_player_id, p_ballot, p_confirmed);
end;
$$;
...
-- Organizer path: a tournament manager may write a ballot for any roster entry
-- in their event. Used by the admin "simulate votes" harness so a whole flow
-- can be tested without 16 logins.
create or replace function public.app_mixer_admin_set_ballot(
  p_round_id uuid,
  p_voter_player_id uuid,
  p_ballot jsonb,
  p_confirmed boolean default null
...
revoke all on function public.app_mixer_admin_set_ballot(uuid, uuid, jsonb, boolean) from public;
grant execute on function public.app_mixer_admin_set_ballot(uuid, uuid, jsonb, boolean) to authenticated;
